...
from autoaugment.domain.vision.classification.metrics import CrossEntropyLabelSmooth
...
class CrossEntropyMixUpLabelSmooth(torch.nn.Module):
    def __init__(self, num_classes, epsilon, reduction='mean'):
        ...
        self.ce = CrossEntropyLabelSmooth(num_classes,
                                          epsilon,
                                          reduction=reduction)

    def forward(self, input, target1, target2, lam):  # pylint: disable=redefined-builtin
        return lam * self.ce(input, target1) + (1 - lam) * self.ce(
            input, target2)
