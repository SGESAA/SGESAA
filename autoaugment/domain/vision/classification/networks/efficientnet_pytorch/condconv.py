...
class CondConv2d(nn.Module):
    ...
    def reset_parameters(self):
        ...
        if self.bias is not None:
            ...
            init_bias = get_condconv_initializer(
                partial(nn.init.constant_, val=0), self.num_experts,
                self.bias_shape)
            init_bias(self.bias)
    ...
    def forward(self, x, routing_weights):
        ...
        weight = torch.matmul(
            routing_weights,
            self.weight)  # (Expert x out x in x 3x3) --> (B x out x in x 3x3)
        ...
        # move batch elements with channels so each batch element can be efficiently convolved with
        # separate kernel
        x = x.view(1, B * C, H, W)
