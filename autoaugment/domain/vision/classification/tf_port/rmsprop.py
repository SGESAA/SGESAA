...
class RMSpropTF(Optimizer):
    ...
    learning rate is thus :math:`\alpha/(\sqrt{v + \epsilon})` where :math:`\alpha` from 
    :math:`\alpha/(\sqrt{v} + \epsilon)` where :math:`\alpha`
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if len(state) == 0:
                    ...
                    state['step'] = 0
                    state['ms'] = torch.ones_like(
                        p.data)  # , memory_format=torch.preserve_format)
                    state['mom'] = torch.zeros_like(
                        p.data)  # , memory_format=torch.preserve_format)
                ...
                mom.mul_(group['momentum']).addcdiv_(
                    group['lr'], grad, (ms + group['eps']).sqrt())
