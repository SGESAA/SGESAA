...
def extract_spectrogram(values, clip, entries):
    for data in entries:
        ...
        for i in range(num_channels):
            window_length = int(
                round(window_sizes[i] * args.sampling_rate / 1000))
...
def extract_features(audios):
    ...
    for audio in audio_names:
        clip, sr = librosa.load("{}/{}".format(args.data_dir, audio),
                                sr=args.sampling_rate)
        entries = audios.loc[audios["filename"] == audio].to_dict(
            orient="records")
...
# python autoaugment/domain/audio/classification/preprocessing/preprocessingESC.py --csv_file /data/torch/datasets/audio/ESC-50/meta/esc50.csv --data_dir /data/torch/datasets/audio/ESC-50/audio --store_dir /data/torch/datasets/audio/ESC-50/spectrograms/ --sampling_rate 44100
