...
class AugmentDataset(torch.utils.data.Dataset):
    def __init__(self, dataset, pre_transforms, after_transforms,
                 valid_transforms, ops_names, search, magnitudes):
...
def get_dataloaders(dataset,
    ...
    if dataset == 'cifar10':
        ...
        valid_idx = list(
            filter(lambda x: total_trainset.targets[x] in idx120, valid_idx))
        test_idx = list(
