...
class ImageNet(torchvision.datasets.ImageFolder):
    ...
    def __init__(self, root, split='train', download=False, **kwargs):
        ...
        if split == 'train' and os.path.exists(listfile):
            ...
            samples = [(os.path.join(self.split_folder, line + '.JPEG'),
                        class_to_idx[line.split('/')[0]]) for line in datalist]
    ...
    def download(self):
        if not check_integrity(self.meta_file):
            ...
            download_and_extract_tar(archive_dict['url'],
                                     self.root,
                                     extract_root=tmpdir,
    ...
    def _verify_split(self, split):
        if split not in self.valid_splits:
            ...
            msg += "Valid splits are {}.".format(", ".join(
                self.valid_splits))
