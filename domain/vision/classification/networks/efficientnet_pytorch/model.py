...
from autoaugment.domain.vision.classification.networks.efficientnet_pytorch.utils import (
    round_filters,
    round_repeats,
    drop_connect,
...
class MBConvBlock(nn.Module):
    ...
    def __init__(self, block_args, global_params, norm_layer=None):
        ...
        Conv2d = get_same_padding_conv2d(
            ...
            condconv_num_expert=block_args.condconv_num_expert)
        ...
        if self._block_args.expand_ratio != 1:
            self._expand_conv = Conv2d(in_channels=inp,
                                       ...
                                       kernel_size=1,
                                       bias=False)
            self._bn0 = norm_layer(num_features=oup,
        ...
        self._depthwise_conv = Conv2d(
            ...
            stride=s,
            bias=False)
        self._bn1 = norm_layer(num_features=oup,
                               momentum=self._bn_mom,
                               eps=self._bn_eps)
        ...
        if self.has_se:
            ...
            self._se_reduce = Conv2dse(in_channels=oup,
        ...
        self._project_conv = Conv2d(in_channels=oup,
                                    ...
                                    kernel_size=1,
                                    bias=False)
        self._bn2 = norm_layer(num_features=final_oup,
                               momentum=self._bn_mom,
                               eps=self._bn_eps)
    ...
    def forward(self, inputs, drop_connect_rate=None):
        ...
        if self._is_condconv():
            ...
            if self._block_args.expand_ratio != 1:
                _expand_conv = partial(self._expand_conv,
                                       routing_weights=routing_w)
            _depthwise_conv = partial(self._depthwise_conv,
                                      routing_weights=routing_w)
            _project_conv = partial(self._project_conv,
                                    routing_weights=routing_w)
        else:
            if self._block_args.expand_ratio != 1:
...
class EfficientNet(nn.Module):
    ...
    def __init__(self, blocks_args=None, global_params=None, norm_layer=None):
        ...
        self._conv_stem = Conv2d(in_channels,
                                 ...
                                 stride=2,
                                 bias=False)
        self._bn0 = norm_layer(num_features=out_channels,
                               momentum=bn_mom,
                               eps=bn_eps)
        ...
        self._conv_head = Conv2d(in_channels,
                                 ...
                                 kernel_size=1,
                                 bias=False)
        self._bn1 = norm_layer(num_features=out_channels,
                               momentum=bn_mom,
                               eps=bn_eps)
