...
from autoaugment.domain.vision.classification.networks.shakedrop import ShakeDrop
...
class PyramidNet(nn.Module):
    def __init__(self, dataset, depth, alpha, num_classes, bottleneck=True):
        ...
        if self.dataset.startswith('cifar'):
            ...
            self.ps_shakedrop = [
                1. - (1.0 - (0.5 / (3 * n)) * (i + 1)) for i in range(3 * n)
            ]
    ...
    def pyramidal_make_layer(self, block, block_depth, stride=1):
        downsample = None
        if stride != 1:
            # or self.inplanes != int(round(featuremap_dim_1st))
        ...
        layers.append(
            block(self.input_featuremap_dim,
