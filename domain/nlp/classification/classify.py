...
from autoaugment.domain.nlp.classification.datasets.preprocess import get_clean_text
...
checkpoint_path = 'checkpoints/checkpoint_fasttext.pth.tar'
...
def prepro_doc(document, word_map):
    ...
    words_per_each_sentence = torch.LongTensor(
        words_per_each_sentence).unsqueeze(0).to(device)  # (1, n_sentences)
...
def prepro_sent(text, word_map):
    ...
    words_per_sentence = torch.LongTensor([words_per_sentence
                                           ]).to(device)  # (1)

    # encode sentence with indices from the word map
    encoded_sent = list(
        map(lambda w: word_map.get(w, word_map['<unk>']),
            sentence)) + [0] * (word_limit - len(sentence))
...
if __name__ == '__main__':
    ...
    text = 'How do computers work? I have a CPU I want to use.'
    'But my keyboard and motherboard do not help.\n\n '
